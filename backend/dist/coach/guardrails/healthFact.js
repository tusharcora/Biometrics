...
exports.classifyHealthFact = classifyHealthFact;
...
function classifyHealthFact(value) {
    const text = normalize(value);
    const categories = Object.keys(PATTERNS).filter((c) => PATTERNS[c].some((re) => re.test(text)));
    if ((0, crisis_1.classifyCrisis)(value).triggered)
        categories.push('crisis_overlap');
    return { blocked: categories.length > 0, categories };
}
//# sourceMappingURL=healthFact.js.map