...
exports.suggestReferences = suggestReferences;
exports.referenceCopiedValues = referenceCopiedValues;
const REF_RE = /\{\{[^{}]*\}\}/g;
...
function suggestReferences(raw, results) {
    ...
    for (const literal of literals) {
        ...
        const matches = (strings.length > 0 ? strings : matching)
            ...
            .map((leaf) => `{{${leaf.path}}}`);
        if (matches.length > 0)
            hints.push({ literal, references: matches });
    }
    return hints;
}
 ...
 */
function referenceCopiedValues(raw, results) {
    const latest = new Map();
    for (const r of results)
        latest.set(r.name, r.result);
    const leaves = [];
    ...
    for (const leaf of leaves) {
        if (typeof leaf.value !== 'string' || !DISPLAY_VALUE_RE.test(leaf.value))
            continue;
        if (!byValue.has(leaf.value))
            byValue.set(leaf.value, leaf.path);
    }
