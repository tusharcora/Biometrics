...
exports.isExplicitMemoryDismissal = isExplicitMemoryDismissal;
exports.classifyMemoryFeedback = classifyMemoryFeedback;
const APOSTROPHES = /[‘’ʼ]/g;
const START = "(?<![\\p{L}\\p{N}'])";
const END = "(?![\\p{L}\\p{N}'])";
const THAT = '(?:that|it|this)';
const EXPLICIT_DISMISSALS = [
    ...
    new RegExp(`${START}that(?:'s| s|s| is| was) (?:not (?:right|correct|true|accurate)|wrong|incorrect|a mistake)${END}`, 'u'),
...
const STOP_WORDS = new Set(('the and for are but you your yours our his her its their them they she him who whom what which when where why how ' +
    ...
    'actually instead rather meant wrong incorrect changed change changes changing').split(' '));
...
function isExplicitMemoryDismissal(message) {
 ...
 */
function classifyMemoryFeedback(message, entryValue = '') {
    if (isExplicitMemoryDismissal(message))
        return 'dismiss';
    const text = normalise(message);
    const tokens = text.match(/\p{L}+/gu) ?? [];
    if (!tokens.some((t) => CORRECTION_CUES.has(t)))
        return 'confirm';
...
//# sourceMappingURL=memoryFeedback.js.map