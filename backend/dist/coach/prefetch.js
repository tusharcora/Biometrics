...
exports.daysFromQuestion = daysFromQuestion;
exports.planPrefetch = planPrefetch;
...
const RANGE_RE = /\b(?:trend(?:ed|ing|s)?|week|weekly|month|monthly|fortnight|lately|recently|over time|average|avg|how many days|best|worst|highest|lowest|last \d+ days|past \d+ days|this year)\b/i;
const HABIT_RE = /\b(?:drink|drank|drinks|drinking|alcohol|beers?|wines?|booze|caffeine|coffees?|espressos?|teas?|cups?|work(?:ed)? out|workouts?|exercis(?:e|ed|ing)|gym|habits?|logged|log)\b/i;
/** How many days the question covers; `fallback` when it names no span. */
function daysFromQuestion(message, fallback, max) {
    const m = message.toLowerCase();
    const explicit = /\b(?:last|past)\s+(\d{1,3})\s+days?\b/.exec(m);
    if (explicit)
        return Math.min(Math.max(Number(explicit[1]), 1), max);
    if (/\b(?:two weeks|2 weeks|fortnight)\b/.test(m))
        return Math.min(14, max);
    if (/\bthis year|past year|last year\b/.test(m))
        return max;
    if (/\bmonth|monthly\b/.test(m))
        return Math.min(30, max);
    if (/\bweek|weekly\b/.test(m))
        return Math.min(7, max);
    return Math.min(fallback, max);
}
function planPrefetch(message) {
    const plan = [];
    ...
    if (metrics.length === 1 && RANGE_RE.test(message)) {
        plan.push({ name: 'getMetricHistory', args: { metric: metrics[0], days: daysFromQuestion(message, 7, tools_1.MAX_HISTORY_DAYS) } });
    }
    if (HABIT_RE.test(message)) {
        plan.push({ name: 'getHabitLogs', args: { days: daysFromQuestion(message, 7, metrics_1.MAX_HABIT_LOG_DAYS) } });
    }
    return plan;
}
