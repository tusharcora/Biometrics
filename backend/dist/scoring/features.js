...
exports.sleepDebtRolling = sleepDebtRolling;
exports.buildSleepDebtSeries = buildSleepDebtSeries;
exports.baselineDeviationPct = baselineDeviationPct;
exports.acuteChronicLoadRatio = acuteChronicLoadRatio;
exports.groupSessionsByNight = groupSessionsByNight;
exports.sleepEfficiency = sleepEfficiency;
exports.buildSleepEfficiencySeries = buildSleepEfficiencySeries;
exports.mainSessionOnsets = mainSessionOnsets;
exports.circadianConsistencyOn = circadianConsistencyOn;
exports.buildCircadianSeries = buildCircadianSeries;
 ...
 */
function sleepDebtRolling(sleep, date, goalMinutes, cfg) {
    const from = (0, dates_1.shiftDate)(date, -(cfg.sleepDebtWindowDays - 1));
    let debt = 0;
    for (const night of sleep) {
        if (night.date >= from && night.date <= date)
            debt += Math.max(0, goalMinutes - night.value);
    }
    return debt;
}
/**
 ...
 */
function buildSleepDebtSeries(sleep, throughDate, goalMinutes, cfg) {
    if (sleep.length === 0)
        return [];
    const firstNight = sleep.reduce((min, p) => (p.date < min ? p.date : min), sleep[0].date);
    const out = [];
    for (let d = (0, dates_1.shiftDate)(firstNight, cfg.sleepDebtWindowDays - 1); d <= throughDate; d = (0, dates_1.shiftDate)(d, 1)) {
        out.push({ date: d, value: sleepDebtRolling(sleep, d, goalMinutes, cfg) });
    }
    return out;
}
/** Percent deviation of `value` from its EWMA baseline. */
function baselineDeviationPct(value, ewma) {
    return ((value - ewma) / ewma) * 100;
}
function meanOver(steps, from, to) {
    let sum = 0;
    let count = 0;
 ...
 */
function acuteChronicLoadRatio(steps, date, cfg) {
    const acute = meanOver(steps, (0, dates_1.shiftDate)(date, -(cfg.acwr.acuteDays - 1)), date);
    const chronic = meanOver(steps, (0, dates_1.shiftDate)(date, -(cfg.acwr.chronicDays - 1)), date);
    if (acute.count === 0 || chronic.count < cfg.acwr.minChronicObservations || chronic.mean === 0)
        return null;
 ...
 * RHR.
 */
function groupSessionsByNight(sessions, timeZone) {
 ...
 */
function sleepEfficiency(nightSessions) {
    let asleep = 0;
    let inBed = 0;
...
function buildSleepEfficiencySeries(sessions, timeZone, throughDate) {
    const out = [];
 ...
 */
function mainSessionOnsets(sessions, timeZone) {
    const out = [];
    for (const [date, night] of groupSessionsByNight(sessions, timeZone)) {
        let main;
        for (const s of night) {
            if (!(intervalMinutes(s) > 0))
                continue;
            if (!main ||
        ...
        if (main)
            out.push({ date, value: (0, civilDate_1.sessionStartMinutesSinceLocalNoon)(main, timeZone) });
    }
    return out.sort((a, b) => (a.date < b.date ? -1 : 1));
}
/**
 ...
 * "starting from today".
 */
function circadianConsistencyOn(onsets, date, cfg) {
    const { windowDays, minWindowNights, minHistoryNights, maxStdMinutes } = cfg.circadian;
    if (onsets.filter((o) => o.date <= date).length < minHistoryNights)
        return null;
    const from = (0, dates_1.shiftDate)(date, -(windowDays - 1));
    const window = onsets.filter((o) => o.date >= from && o.date <= date).map((o) => o.value);
    if (window.length < minWindowNights)
        return null;
 ...
 * sleep-debt series.
 */
function buildCircadianSeries(onsets, throughDate, cfg) {
    const sorted = [...onsets].sort((a, b) => (a.date < b.date ? -1 : 1));
    if (sorted.length < cfg.circadian.minHistoryNights)
        return [];
    const out = [];
    for (let d = sorted[cfg.circadian.minHistoryNights - 1].date; d <= throughDate; d = (0, dates_1.shiftDate)(d, 1)) {
